...
# Внутри new_arch dataset_pipeline указывает на локальные папки new_arch/dataset
# и используется как самодостаточная копия (без зависимостей от старого dataset_pipeline).
_NEW_ARCH_ROOT = Path(__file__).resolve().parents[1]
DEFAULT_DATA_DIR = _NEW_ARCH_ROOT.parent / "data"   # сырые subject-папки остаются в data/
DEFAULT_DATASET_DIR = _NEW_ARCH_ROOT / "dataset"    # все parquet'ы внутри new_arch/dataset
