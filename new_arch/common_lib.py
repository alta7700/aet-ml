...
PREDICTIONS_REQUIRED_COLUMNS: list[str] = [
    "model_id", "fold_id", "subject_id", "epoch",
...
def build_checkpoint_filename(meta: ExperimentMetadata, fold_id: str, ext: str,
    ...
    """Имя checkpoint-файла per-fold (single).
...
def build_grouped_checkpoint_filename(meta: ExperimentMetadata, epoch: int,
                                      ext: str = "pt") -> str:
    """Имя сгруппированного checkpoint-файла per-epoch.

    model_{model_id}_epoch-{NN}.{ext}, где ext — pt (torch) или joblib (sklearn).
    Содержит dict {fold_id: <state_dict | pipeline>} — все LOSO-folds на одной эпохе.
    Для sklearn-моделей epoch=0 (модели не имеют эпох).
    """
    return f"model_{meta.model_id}_epoch-{int(epoch):03d}.{ext}"
...
def save_models_csv(meta: ExperimentMetadata, results_root: Path) -> Path:
    ...
    Concurrent-safe: read-modify-write обёрнут в fcntl.flock на отдельном
    .lock-файле, чтобы несколько процессов могли безопасно писать в один
    models.csv (нужно при параллельном запуске через orchestrator).
    """
...
def save_grouped_checkpoint(states_by_fold: dict[str, Any],
                            model_dir_path: Path,
                            meta: ExperimentMetadata,
                            epoch: int,
                            *, backend: Literal["torch", "joblib"] = "torch") -> Path:
    """Сохраняет состояния всех LOSO-folds на одной эпохе в один файл.

    backend="torch":   .pt через torch.save (для nn.Module.state_dict()).
    backend="joblib":  .joblib через joblib.dump (для sklearn pipelines).

    На вход:
      states_by_fold = {fold_id: state_dict_или_pipeline, ...}
    Результат:
      results/{architecture_id}/{model_id}/model_{model_id}_epoch-{NN}.{pt|joblib}
    Загрузка:
      torch:  states = torch.load(path, weights_only=True, map_location='cpu')
      joblib: states = joblib.load(path)
    """
    model_dir_path = Path(model_dir_path)
    model_dir_path.mkdir(parents=True, exist_ok=True)
