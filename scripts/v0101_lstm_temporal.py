...
class Config:
    ...
    seq_length    = 15      # 75 сек контекста (15 окон × 5 сек)
    batch_size    = 16
    num_epochs    = 100
...
class TemporalWindowDataset(Dataset):
    """Скользящие окна (шаг=1) для LSTM — больше обучающих примеров."""

    def __init__(self, X: np.ndarray, y: np.ndarray, seq_length: int):
        self.X = X
        self.y = y
        self.seq_length = seq_length
        # Первое окно заканчивается на индексе seq_length-1
        self.n = max(0, len(X) - seq_length + 1)

    def __len__(self) -> int:
        return self.n

    def __getitem__(self, idx: int) -> tuple[torch.Tensor, torch.Tensor]:
        X_seq = torch.from_numpy(self.X[idx: idx + self.seq_length]).float()
        y_target = torch.tensor(self.y[idx + self.seq_length - 1], dtype=torch.float32)
...
def _run_one_loso_fold(test_subject_id,
    ...
    # y_pred_sparse: предсказания только для окон [seq_length-1 .. len(y_te)-1]
    y_pred_sparse = np.mean(fold_preds, axis=0)

    # Дополняем начало: первые seq_length-1 точек не предсказаны → берём первое значение
    n_pad = len(y_te) - len(y_pred_sparse)
    if n_pad > 0:
        y_pred_full = np.concatenate([np.full(n_pad, y_pred_sparse[0]), y_pred_sparse])
    else:
...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(description="v0101 — LSTM/GRU temporal ensemble")
...
def main() -> None:
    ...
    print("=" * 70)
    print("v0101 — LSTM/GRU TEMPORAL ENSEMBLE (rev2)")
    print("=" * 70)
    print(f"seq_length={config.seq_length}  ({config.seq_length*5} сек контекста)")
