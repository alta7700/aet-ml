...
Результаты: results/v0007/

Идея:
  Тестируем 6 независимых стратегий добавления признаков вариабельности к базе v0004.
  Каждая стратегия использует ОДИН масштаб — разные масштабы в одну модель не смешиваем.
  Для каждой стратегии проверяем весь зоопарк моделей.

  Стратегии:
    1. Trend CV  30s  — slope и ratio CV длительностей в 30-секундном окне
    2. Trend CV  60s  — то же, но по циклам за последние 60с
    3. Trend CV 120s  — то же, но за последние 120с
    4. SampEn    30s  — непредсказуемость ритма в 30-секундном окне
    5. SampEn    60s  — то же, 60с
    6. SampEn   120s  — то же, 120с

  Зоопарк моделей:
    Ridge α=100, Huber ε=1.35, ElasticNet, SVR (rbf), RandomForest, GradBoost

Воспроизведение:
  uv run python scripts/v0007_train_variability_features.py
  uv run python scripts/v0007_train_variability_features.py --target lt1
  uv run python scripts/v0007_train_variability_features.py --no-plots
"""
...
# ─── Базовые наборы признаков (из v0004) ─────────────────────────────────────
...
def _trend_cv_feats(scale: str) -> list[str]:
    return [
...
def _sampen_feats(scale: str) -> list[str]:
    return [f"load_sampen_{scale}", f"rest_sampen_{scale}"]
...
def print_zoo_table(
    ...
    """Печатает таблицу (конфиг × модель) с MAE. Возвращает (best_config, best_model)."""
    model_names = list(ZOO.keys())
    col_w = 12  # ширина колонки модели

    # Заголовок
    header = f"  {'Конфигурация':<22s}" + "".join(f"{m:>{col_w}s}" for m in model_names)
    print(header)
    ...
    for cfg in configs:
        ...
        for mdl in model_names:
            ...
            row += f"{mae:>{col_w}.3f}"
        # пометим строку если лучше базы
        delta = min(results_matrix[cfg][m]["mae_min"] for m in model_names) - base_mae
        marker = " ★" if delta < -1e-4 else "  "
        print(row + f"   Δmin={delta:+.3f}{marker}")

    print("  " + "─" * (22 + col_w * len(model_names)))
    print(f"\n  ★ ЛУЧШАЯ: {best_cfg} + {best_mdl}  MAE={best_mae:.3f}  Δ={best_mae - base_mae:+.3f}")
    return best_cfg, best_mdl


# ─── Визуализация ─────────────────────────────────────────────────────────────

def plot_predicted_vs_actual(result: dict, df: pd.DataFrame, title: str, output_path: Path) -> None:
    subjects = sorted(df["subject_id"].unique())
    ...
    mae = result["mae_min"]; r2 = result["r2"]; rho = result["rho"]
    ...
    ax2.set_xlabel("MAE, мин"); ax2.set_title("MAE по участникам")
...
def run_lt2(df_full: pd.DataFrame, no_plots: bool = False) -> dict:
    print("\n" + "═" * 75)
    print("LT2 — 6 стратегий вариабельности × зоопарк моделей")
    print("═" * 75)
    ...
    configs: dict[str, list[str]] = {
        ...
        "Trend CV  30s":    base + _trend_cv_feats("30s"),
        "Trend CV  60s":    base + _trend_cv_feats("60s"),
        "Trend CV 120s":    base + _trend_cv_feats("120s"),
        "SampEn    30s":    base + _sampen_feats("30s"),
        "SampEn    60s":    base + _sampen_feats("60s"),
        "SampEn   120s":    base + _sampen_feats("120s"),
    }
    ...
    for cfg, feats in configs.items():
        ...
        for mdl_name, mdl_factory in ZOO.items():
            ...
            print(f"  [{done:2d}/{total}] {cfg:<22s} + {mdl_name:<13s}", end="", flush=True)
            r = loso(df, avail, target, mdl_factory)
            results_matrix[cfg][mdl_name] = r
            print(f"  MAE={r['mae_min']:.3f}")

    print()
    best_cfg, best_mdl = print_zoo_table(results_matrix, list(configs.keys()), b_mae)
    ...
    if not no_plots:
        plot_predicted_vs_actual(
            best_result, df,
            f"LT2 v0007: {best_cfg} + {best_mdl}",
            OUT_DIR / "lt2" / "lt2_best.png",
        )

    # репликация v0004 на Ridge для совместимости с save_summary
    base_result = results_matrix["v0004-репликация"]["Ridge α=100"]
    return {
        "best_cfg": best_cfg, "best_mdl": best_mdl,
        "best_result": best_result,
        "results_matrix": results_matrix,
        "df": df,
        "base_mae": base_result["mae_min"],
    }
...
def run_lt1(df_full: pd.DataFrame, no_plots: bool = False) -> dict:
    print("\n" + "═" * 75)
    print("LT1 — 6 стратегий вариабельности × зоопарк моделей")
    print("═" * 75)
    ...
    if not no_plots:
        plot_predicted_vs_actual(
            best_result, df,
            f"LT1 v0007: {best_cfg} + {best_mdl}",
            OUT_DIR / "lt1" / "lt1_best.png",
        )

    base_result = results_matrix["v0004-репликация"]["Huber ε=1.35"]
    return {
        "best_cfg": best_cfg, "best_mdl": best_mdl,
        "best_result": best_result,
        "results_matrix": results_matrix,
...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(description="v0007: 6 стратегий вариабельности × зоопарк моделей.")
...
def main() -> None:
    args = parse_args()
    print("=" * 75)
    print("v0007 — СТРАТЕГИИ ВАРИАБЕЛЬНОСТИ × ЗООПАРК МОДЕЛЕЙ")
    print("=" * 75)
