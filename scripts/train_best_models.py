"""Канонический скрипт воспроизведения лучших моделей.

Реализует финальные конфигурации для LT2 и LT1:

  LT2 (13 участников, LOSO):
    NIRS + HRV + interaction (SmO2×RR) + session-z EMG
    Модель: Ridge(alpha=100) — устойчивее ElasticNet на LOSO с 13 субъектами
    Целевой результат: MAE ≈ 2.097 мин, R² ≈ 0.848, ρ ≈ 0.924

  LT1 (9 участников, LOSO):
    Session-z EMG + HRV
    Модель: HuberRegressor(epsilon=1.35) — устойчива к S003 (элитный атлет, выброс)
    Целевой результат: MAE ≈ 1.986 мин, R² ≈ 0.841, ρ ≈ 0.932

Ключевые идеи:
  - Session-z нормировка EMG: убирает межсубъектные различия в абсолютных значениях
    ЭМГ (зависят от электрода/кожи), сохраняет форму траектории внутри сессии.
    Применяется ТОЛЬКО к ЭМГ (не к NIRS, где % имеет физический смысл).
  - Running NIRS признаки: smo2_from_running_max решает проблему того, что у разных
    субъектов LT1 наступает при разном абсолютном SmO₂, но ρ(SmO₂, LT1) = 0.968
    внутри субъекта.
  - Interaction SmO2×RR: произведение двух сигналов из разных систем (мышца + сердце).
  - Huber для LT1: S003 — элитный атлет с нестандартной физиологией (выброс).
    HuberRegressor снижает его влияние без исключения из датасета.
  - hrv_mean_rr_ms несёт и динамику внутри сессии, и межсубъектную тренированность
    (between-subject confound); это документировано и принимается как компромисс
    при малом N=9–13.
...
OUT_DIR = _ROOT / "results" / "best_models"
