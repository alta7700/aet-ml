"""v0106a — Wavelet-features → Attention-LSTM

Версия:    v0106a
Дата:      2026-05-08

Гибрид: CWT скалограммы (из v0103) на вход Attention-LSTM (из v0104).

Pipeline:
  Последовательность окон (seq_len × n_features)
      ↓
  CWT по каждому признаку → cwt_energy (seq_len × n_features × n_scales)
  Линейная проекция: (n_features × n_scales) → feat_embed=16 на каждом шаге
      ↓
  Конкатенация с оригинальными признаками: (n_features + feat_embed) per step
      ↓
  LSTM (2 слоя, hidden=64) → Multi-Head Attention pooling
      ↓
  Dropout → Linear → предсказание

Логика: Attention-LSTM (v0104) был лучшим из NN (4.03 мин).
        CWT дала улучшение над raw features в v0103.
        Объединяем: LSTM видит частотный состав сигнала + оригинальные признаки.
"""
...
class WaveletAttentionLSTM(nn.Module):
    """CWT-проекция + конкатенация с orig features + LSTM + Attention pooling."""

    def __init__(self, n_features: int, n_cwt: int, hidden_size: int = 64,
                 num_layers: int = 2, num_heads: int = 4,
                 feat_embed: int = 16, dropout: float = 0.3):
        super().__init__()
    ...
    def forward(self, x_orig: torch.Tensor, x_cwt: torch.Tensor) -> torch.Tensor:
        ...
        x_cwt_proj = self.cwt_proj(x_cwt)                   # (batch, seq_len, feat_embed)
        x_in = torch.cat([x_orig, x_cwt_proj], dim=-1)      # (batch, seq_len, n_f+feat_embed)
...
def main():
    ...
    print("=" * 70)
    print("v0106a — WAVELET → ATTENTION-LSTM")
    print("=" * 70)
