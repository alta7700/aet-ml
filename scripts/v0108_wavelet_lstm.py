"""v0108 — Wavelet-features → plain LSTM (last-token head)

Версия:    v0108
Базируется на: v0106a (Wavelet-Attention-LSTM)
Дата:      2026-05-16

Чистая связка: CWT-фичи + LSTM с последним токеном (как в v0101) —
без Attention, без TCN, без CNN. Эта комбинация в проекте не
проверялась, хотя v0101 (LSTM 75 сек) — сильнейший NN на этой задаче,
а добавление wavelet-фич концептуально должно дать преимущество над
исходными признаками.

Pipeline:
  Последовательность окон (seq_len × n_features)
      ↓
  CWT по каждому признаку → cwt_energy (seq_len × n_features × n_scales)
  Линейная проекция: (n_features × n_scales) → feat_embed=16 на шаге
      ↓
  Конкатенация с оригинальными признаками: (n_features + feat_embed) per step
      ↓
  LSTM (2 слоя, hidden=64) → последний токен → Dropout → Linear → ŷ

Отличие от v0106a: вместо AttentionPooling на голове LSTM — обычный
последний токен (last_hidden), как в победившем v0101.
"""
...
class Config:
    ...
    wavelet       = "morl"
    scales        = [1, 2, 4, 8, 16]
    feat_embed    = 16    # проекция CWT: n_features×n_scales → 16
    hidden_size   = 64
    num_layers    = 2
    num_heads     = 4
    dropout       = 0.3
...
class WaveletLSTM(nn.Module):
    """CWT-проекция + конкатенация с orig features + LSTM (last token).

    Прямой аналог v0101 (plain LSTM, last-hidden head), но на входе —
    оригинальные признаки И их CWT-разложение. Сравнение с v0101 на
    одинаковом seq_length показывает чистый вклад wavelet-фич.
    """

    def __init__(self, n_features: int, n_cwt: int, hidden_size: int = 64,
                 num_layers: int = 2, feat_embed: int = 16,
                 dropout: float = 0.3):
        super().__init__()
        self.cwt_proj = nn.Sequential(
            nn.Linear(n_cwt, feat_embed),
            nn.ReLU(),
            nn.Dropout(dropout),
        )
        self.lstm = nn.LSTM(
    ...
    def forward(self, x_orig: torch.Tensor, x_cwt: torch.Tensor) -> torch.Tensor:
        ...
        last = lstm_out[:, -1, :]                # last-token head
...
def _run_one_loso_fold(test_subject_id, df, feat_cols, target_col, config,
    ...
    model = WaveletLSTM(
        ...
        feat_embed=config.feat_embed, dropout=config.dropout,
    ).to(config.device)
...
def main():
    ...
    p.add_argument("--seq-len",     type=int, default=6,
                   help="Длина последовательности в шагах (по 30 сек). "
                        "Дефолт 6 → 3 мин контекста (sweet-spot из LSTM-абляции).")
    p.add_argument("--window-step", type=int, default=6,
                   help="Шаг прореживания × 5 сек. Дефолт 6 → 30 сек.")
    ...
    print("=" * 70)
    print("v0108 — WAVELET → LSTM (last-token head)")
    print("=" * 70)
