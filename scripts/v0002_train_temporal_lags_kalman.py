...
Результаты: results/v0002/

Что изменено по сравнению с v0001:
  - Добавлены лаговые признаки (t-1, t-2, t-3) и дельты — ИТОГ: не помогают
  - Добавлено Kalman-сглаживание как постобработка LOSO-предсказаний

Гиперпараметры:
  Базовая модель: ElasticNet(alpha=0.5, l1_ratio=0.9, max_iter=5000, random_state=42)
  Imputer:        SimpleImputer(strategy="median")
  Scaler:         StandardScaler(with_mean=True, with_std=True)
  Kalman sigma_process: 30.0 с  (физический prior: порог убывает ~30 с/окно)
  Kalman sigma_obs:     120.0 с (доверие к модели; ≈ 2 мин ≈ 2×LOSO MAE)
  Kalman: детерминирован, seed не нужен

Ожидаемые результаты:
  Без Kalman (ElasticNet baseline):  LT2=2.110 мин, LT1=2.030 мин
  С Kalman (без лагов):              LT2=2.054 мин, LT1=2.007 мин
  Лаги t-1 + Kalman:                 LT2=2.083 мин, LT1=2.008 мин
  Вывод: лаги не помогают, Kalman даёт ~0.02–0.06 мин стабильного улучшения

Воспроизведение:
  uv run python scripts/v0002_train_temporal_lags_kalman.py
  uv run python scripts/v0002_train_temporal_lags_kalman.py --target lt1
  uv run python scripts/v0002_train_temporal_lags_kalman.py --no-kalman
  uv run python scripts/v0002_train_temporal_lags_kalman.py --no-plots
"""
