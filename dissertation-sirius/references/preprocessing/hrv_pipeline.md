...
ML-окно: **30 с, шаг 5 с**.

HRV-признаки вычисляются по **trailing-контексту 120 с** — берётся 120 с RR-интервалов, накопленных до конца текущего окна (каузально). Это отдельный параметр: `DEFAULT_HRV_CONTEXT_SEC = 120.0`.

| Признак | Минимум RR в контексте | При ЧСС 120 = сек |
|---|---|---|
| mean_rr, rmssd | 5 | ~2.5 сек |
| DFA-α1 (n=4–16) | 32 (n_max × 2) | ~16 сек |
| sd1, sd2 | 4 | ~2 сек |

При контексте 120 с и ЧСС ≥ 60 уд/мин: ≥ 120 RR → DFA-α1 считается надёжно.
...
| Параметр | Значение |
|---|---|
| Порог артефакта | ±20% от скользящей медианы (окно 5 ударов) |
| Абсолютные границы | 300–2000 мс |
| Замена артефактов | Кубическая интерполяция по валидным |
| DFA масштабы | n = 4–16 кардиоциклов (Gronwald & Rogers, 2019–2022) |
| ML-окно | 30 с, шаг 5 с |
| HRV trailing context | **120 с** (каузальный) |
| Интерполяция | Только для LF/HF (не используется в baseline) |
